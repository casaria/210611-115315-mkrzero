...
NexDSButton bt0 = NexDSButton(0, 26, "bt0"); // Button added
NexDSButton bt1 = NexDSButton(0, 18, "bt1"); // Button added
NexDSButton bt2 = NexDSButton(0, 19, "bt2"); // Button added
NexDSButton bt3 = NexDSButton(0, 20, "bt3"); // Button added
NexDSButton bt4 = NexDSButton(0, 21, "bt4"); // Button added
NexDSButton bt5 = NexDSButton(0, 22, "bt5"); // Button added
NexDSButton bt8 = NexDSButton(0, 23, "bt8"); // Button added
NexDSButton bt7 = NexDSButton(0, 25, "bt7"); // Button added
/*
...
*/
NexButton b5 = NexButton(1, 4, "b5"); // pAGE RERTURN TO page0
NexButton b6 = NexButton(0, 3, "b6"); // UP button  added
...
uint32_t bt0state = 0; // Create variable to store value we are going to get
...
uint32_t bt8state = 0; // Create variable to store value we are going to get
uint32_t bt7state = 0; // Create variable to store value we are going to get
...
*/
void DimBacklight(int percent)
{  String aString;

    if((percent > 100) || (percent <0)) return;
    aString = "dim=" + String(percent);
    Serial1.print(aString);
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);
}
...
void ChemicalIndicator(bool vis)
{   static bool prev = false;

    if (prev == vis) return;
    prev = vis;

    Serial1.write("vis t13,"); 
    Serial1.write(0x30 + (uint8_t)vis);
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);
...
void ChemicalWarning(bool vis)
{   static bool prev = false;

    if (prev == vis) return;
    prev = vis;

    Serial1.write("vis t15,"); 
    Serial1.write(0x30 + (uint8_t)vis);
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);
...
void updateFromPage1()
{
    ...
    bt0.getValue(&bt0state);
    bt8.getValue(&bt8state);
    bt7.getValue(&bt7state);
    ...
    bt0latch = (uint8_t)bt0state;
    bt8latch = (uint8_t)bt8state;
    bt7latch = (uint8_t)bt7state;
...
{
    bt8latch = !bt8latch;
    bt8state = (uint8_t)bt8latch;
    processPumpCirc(bt8state);
}
...
void start_flash_temp()
{
    ...
    timer1.in(6000, stop_flash_temp);
}
...
void initPage0()
{
    ...
    Serial1.write("vis t13,0"); //  nextion chemcals added syumbol
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);


    Serial1.write("vis t15,0"); //  nextion chemcals added bannerr
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);
...
{ // Put your setup code here, to run once:
    ...
    P1.writeDiscrete(LOW, 1, relayPumpCirc); 
    bt7latch = true;
    CircIndicator(bt7latch);
    ...
    bt0.attachPop(bt0PopCallback);   // Button press
    ...
    bt8.attachPush(bt8PushCallback); // Button press
    bt8.attachPop(bt8PopCallback);
    bt7.attachPush(bt7PushCallback); // Button press
...
void writeHeaterTemp(float value)
{
    ...
    // After the name of the oect you need to put the dot val because val is the atribute we want to change on that object.
    Serial1.print("theater.txt="); // This is) sent to the nextion display to set what object name (before the dot) and what atribute (after the dot) are you going to change.
    Serial1.print("\"" + String(value, 2) + "\"");
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);
...
void writeAmbTemp(float value)
{

    // After the name of the oect you need to put the dot val because val is the atribute we want to change on that object.
    Serial1.print("tamb.txt="); // This is) sent to the nextion display to set what object name (before the dot) and what atribute (after the dot) are you going to change.
    Serial1.print("\"" + String(value, 2) + "\"");
    Serial1.write(0xff);
    Serial1.write(0xff);
    Serial1.write(0xff);
...
{ // the loop routine runs over and over again forever:
    ...
    if (haveData)
    {
        ...
        tempRXcount++;
        if (!tempRXCheck)
            tempRXCheck = timeNow + 7000 + tempRXInterval;
        if (tempRXcount == 4)
            tempRXTimeLeft = tempRXCheck - millis();
    }
...
*/
    ...
    if (StatusUpdtTime < timeNow)
    {
        ...
        StatusText("Everything is great");
        ...
        Serial.println("bt7latch | flagflow | tempePlausible | tempRXcount | tempRXTimeleft");
